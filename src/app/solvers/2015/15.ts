...
export const solver201515: Solver = {
...
function recipes(amount: number, ingredients: number): number[][] {
    // start from first ingredient in all possible quantities
    let res: number[][] = Array.from(Array(amount + 1).keys()).map(key => [key]);
...
function totalScore(recipe: number[], ingredientProperties: number[][]): number {
    let score = 1;
...
function calories(recipe: number[], ingredientProperties: number[][]): number {
